...
* Some way to deal with multiple paths.  Probably just iterate over them
  and do the same operations on each.
...
* Fix SVG units handling.  When the SVG uses mm everything's fine, but
  ...
  and the result is the wrong size.  Along with this, drop cairo and
  use svgpathtools.svg2paths2() instead, and get svg attributes out of [2]
...
* Make the runtests script parallelize the tests.  Or make s2g parallelize
  the work.
...
I currently use svgpathtools (and cairosvg a little bit) to do two things:

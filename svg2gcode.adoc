...
*ramp-slope* (float, optional):: If specified, `ramp-slope` enables ramp
entry into engraving cuts.  The value is the Z depth per unit horizontal
distance of the ramp entry move.  The also enables a move at the end
of the engrave path to remove the entry ramp.  The ramp entry move
may wrap around the path.  If `ramp-slope` is not specified, `engrave`
uses a full depth plunge entry.
